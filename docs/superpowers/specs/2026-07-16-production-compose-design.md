...
Create a standalone `docker-compose.prod.yml` that builds the existing backend and a production frontend image. Only Caddy publishes a host port; PostgreSQL and FastAPI remain on the private Compose network. Caddy serves the compiled React SPA, falls back to `index.html`, and proxies `/api/` to FastAPI with immediate response flushing for server-sent events.
